...
public class SetterCall {
	...
	private static Object dataBind(Map<String, String> map, Class<?> clazz) throws Exception {
		// 1. MyDate인스턴스 생성
//		Object obj = clazz.newInstance(); // deprecated method -- 닫힌 메서드
		Object obj = clazz.getDeclaredConstructor().newInstance(new Object[0]);
		//getDeclaredConstructor(parameterTypes) : 모든 생성자에 접근 
		//(단일 매게 변수 필요). 단, setAccessible(true) 필요
...
//		  Java Reflection을 사용하면 특정 인스턴스의 멤버변수, 메소드 등에 접근할 수 있다.
//		  멤버변수에 접근할 수 있는 방법으로 getFields 메소드를 사용하는데, 이는 public 멤버변수에만 접근이 가능하다.
//		  private을 포함한 모든 멤버변수에 접근하기 위해선 getDeclaredFields 메소드를 사용한다.
