...
public class MyDispatcherServlet extends HttpServlet {
	//Servlet은 클래스 단위로만 맵핑이 된다. 메서드 단위로는 불가능하므로 클래스를 많이 만들어야한다는 단점이 있다
	//HttpServlet을 항상 상속받아야하고 메서드도 service로 고정이다
	//request와 response을 매개변수로 고정으로 받아야한다
	@Override
		...
		try {
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					// getParameterMap()에서 꺼낸 value는 String배열이므로 변환 필요 
					// value가 배열인 이유는 year=2023&year=2022가 가능하다 key값의 중복이 가능하기 때문이다
